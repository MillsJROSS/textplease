...
RSpec.describe "Games", type: :request, sign_in: :user do
  ...
  describe 'POST /games' do
    ...
    it 'prevents tampering with created_by_id' do
      ...
      expect(flash.alert).to eq(t('global.pundit.unauthorize'))
    end
  ...
  describe 'GET /game/:id' do
    it 'allows user to do something' do
      get new_game_path

      expect(response).to have_http_status(200)
      expect(response.body).to have_selector("#game_created_by_id[value=#{@user.id}]", visible: false)
    end
  end
end
